    ...
    *{
    ...
    def eventFilter(self, source, event):
        if (event.type() == QEvent.ContextMenu and source is self.listWidget_2):
            ...
            menu.addAction('editar',lambda : self.editTag(self.ListTags[source.currentRow()]))
            menu.addAction('eliminar',lambda : self.deleteTag(self.ListTags[source.currentRow()]))
            menu.exec_(event.globalPos())
                         
            return True
        return super(NotasApp, self).eventFilter(source, event)    
        
    def editTag(self,tag):
        dialog = Dialog(self)  # self hace referencia al padre
        dialog.tags(self.usuario,tag)
        dialog.show() 
    
    def deleteTag(self,tag):
        self.showdialog("¿Desea eliminar este tag "+tag[2]+" ?","Eliminacion",None,'acep')
        if self.valor=='&Yes':
            self.cur.execute("DELETE FROM tags WHERE id='%s'"%tag[0])
            self.conn.commit()
            self.showdialog("se ha eliminado el Tag "+tag[2],"Eliminacion",None,'info')
...
class Dialog(QDialog):
    ...
    def tags(self,usuario,tags=None):
        ...
        if not tags:
            ...
            self.nombre.setText(tags[2])
            self.btnguardar.clicked.connect(lambda x : self.actualizar_tags(tags,usuario))
    ...
    def actualizar_tags(self,tag,usuario):
       
        if (self.nombre.text()!="" ):
            self.cur.execute("UPDATE  tags SET nombre_tags='{}'  where id='{}'".format(self.nombre.text(),tag[0]))
            self.conn.commit()
            self.main.tags(usuario)
            self.main.showdialog("tag agregado","info",None,'info')
            self.hide()
